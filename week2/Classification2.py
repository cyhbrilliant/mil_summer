...
W1=np.random.randn(30,50)/100.
W2=np.random.randn(50,1)/100.
B1=np.zeros([1,50])
B2=np.zeros([1,1])
...
for i in range(50000):
    ...
    W2 = W2 + lr * (np.dot(ERR.transpose(), L1OUT)).transpose() - 0.0000001*W2
    B2 = B2 + lr * np.sum(ERR)
    # W1 = W1 + lr * (np.dot((np.dot(ERR, W2.transpose())*Dsigmoid(L1OUT)).transpose(), trainimg)).transpose()
    W1 = W1 + lr * (np.dot((np.dot(ERR, W2.transpose())*Dsigmoid(L1OUT)).transpose(), trainimg)).transpose() - 0.0000001*W1
