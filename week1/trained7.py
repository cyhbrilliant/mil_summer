...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(256*49, 512)
    ...
    def forward(self, x):
        ...
        x = self.dropout3(F.relu(self.bn3(self.conv6(x)))).view([-1, 256*49])
        x = F.relu(self.fc1(x))
        feature = x
        x = self.dropout4(x)
        x = self.fc2(x)
...
if IsTraining:
    if IsLoad_state_dict:
        net.load_state_dict(torch.load('./7.pkl')) #载入参数
    ...
    for iter in range(5000000):
        ...
        if (iter+1)%1000==0:
            torch.save(net.state_dict(), './7.pkl')
            print('Save Model')
    ...
    if IsGpuEval:
        ...
        net.load_state_dict(torch.load('./7.pkl', map_location=lambda storage, loc: storage))  # 载入参数
        net.eval()
