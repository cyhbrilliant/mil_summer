...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(128*49, 512)
    ...
    def forward(self, x):
        ...
        x = self.dropout2(self.pool2(F.relu(self.bn2(self.conv4(x))))).view([-1, 128*49])
        x = F.relu(self.fc1(x))
        feature = x
        x = self.dropout3(x)
        x = self.fc2(x)
...
if IsTraining:
    if IsLoad_state_dict:
        net.load_state_dict(torch.load('./9.pkl')) #载入参数
    ...
    for iter in range(5000000):
        ...
        if (iter+1)%1000==0:
            torch.save(net.state_dict(), './9.pkl')
            print('Save Model')
    ...
    if IsGpuEval:
        ...
        net.load_state_dict(torch.load('./9.pkl', map_location=lambda storage, loc: storage))  # 载入参数
        net.eval()
    ...
    Data_batch,Label_batch=getBatchPic(5000,10000)
