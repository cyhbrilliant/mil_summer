...
if IsTraining:
    if IsLoad_state_dict:
        net.load_state_dict(torch.load('./8.pkl')) #载入参数
    ...
    for iter in range(5000000):
        ...
        if (iter+1)%1000==0:
            torch.save(net.state_dict(), './8.pkl')
            print('Save Model')
    ...
    if IsGpuEval:
        ...
        net.load_state_dict(torch.load('./8.pkl', map_location=lambda storage, loc: storage))  # 载入参数
        net.eval()
