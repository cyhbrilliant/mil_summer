...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv2 = torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size=5, stride=1, padding=2)
        self.bn1 = torch.nn.BatchNorm2d(num_features=32)
        self.pool1 = torch.nn.MaxPool2d(kernel_size=2, stride=2)
        self.dropout1 = torch.nn.Dropout2d(0.3)

        self.conv4 = torch.nn.Conv2d(in_channels=32, out_channels=128, kernel_size=5, stride=1, padding=2)
        self.bn2 = torch.nn.BatchNorm2d(num_features=128)
        self.pool2 = torch.nn.MaxPool2d(kernel_size=2, stride=2)
        self.dropout2 = torch.nn.Dropout2d(0.3)

        self.conv6 = torch.nn.Conv2d(in_channels=128, out_channels=256, kernel_size=5, stride=1, padding=2)
...
if IsTraining:
    if IsLoad_state_dict:
        net.load_state_dict(torch.load('./5.pkl')) #载入参数
    ...
    for iter in range(5000000):
        ...
        if (iter+1)%1000==0:
            torch.save(net.state_dict(), './5.pkl')
            print('Save Model')
    ...
    if IsGpuEval:
        ...
        net.load_state_dict(torch.load('./5.pkl', map_location=lambda storage, loc: storage))  # 载入参数
        net.eval()
    ...
    Data_batch,Label_batch=getBatchPic(0, 100)
