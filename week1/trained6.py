...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv2 = torch.nn.Conv2d(in_channels=32, out_channels=32, kernel_size=5, stride=2, padding=2)
    ...
    def forward(self, x):
        ...
        x = self.dropout1(F.relu(self.bn1(self.conv2(x))))
        x = F.relu(self.conv3(x))
        x = self.dropout2(F.relu(self.bn2(self.conv4(x))))
...
if IsTraining:
    if IsLoad_state_dict:
        net.load_state_dict(torch.load('./6.pkl')) #载入参数
    ...
    for iter in range(5000000):
        ...
        if (iter+1)%1000==0:
            torch.save(net.state_dict(), './6.pkl')
            print('Save Model')
    ...
    if IsGpuEval:
        ...
        net.load_state_dict(torch.load('./6.pkl', map_location=lambda storage, loc: storage))  # 载入参数
        net.eval()
